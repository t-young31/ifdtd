...
Additionally, generalised functions for performing the 2D interpolation is not pretty code to look at (although is theoretically possible if we wanted a slight speedup).
This forces us to pass in I,J,K to _every_ component function, rather than only the number of cells in the appropriate direction.

If we want to interpolate all the H-field components, there are functions for this which call the individual component functions separately.
*/
